---
name: <%* 
  const termName = await tp.system.prompt("请输入术语名称（必填）", "");
  if (!termName || !termName.trim()) {
    const currentFile = tp.file.find_tfile(tp.file.path(true));
    if (currentFile) { await app.vault.trash(currentFile, true); }
    new Notice("未输入术语名称，已取消创建并删除草稿。");
    throw new Error("未输入术语名称，已取消。");
  }
  
  const trimmedName = termName.trim();
  
  // 重命名文件
  if (trimmedName !== tp.file.title) {
    try {
      await tp.file.rename(trimmedName);
    } catch (error) {
      const currentFile = tp.file.find_tfile(tp.file.path(true));
      if (currentFile) { await app.vault.trash(currentFile, true); }
      new Notice("重命名失败，已删除草稿：" + error.message);
      throw error;
    }
  }
  
  tR += trimmedName;
%>
<%* 
  const aliasesInput = await tp.system.prompt("请输入术语别名，多个别名用逗号分隔，留空则跳过", "");
  if (aliasesInput && aliasesInput.trim()) {
    const aliases = aliasesInput.split(/[,，]/).map(a => a.trim()).filter(a => a);
    if (aliases.length > 0) {
      tR += "aliases:\n";
      aliases.forEach(alias => {
        tR += `  - ${alias}\n`;
      });
    }
  }
  tR += "tags:\n  - concept\n";
%>
---

## Description

> [!seealso]+ Source
> 💡 **本概念是手动添加的**

> [!note] Definition
> 
<%* 
  const definition = await tp.system.prompt("请输入术语的定义说明，单行输入，如需多行请在生成后手动编辑", "");
  if (definition && definition.trim()) {
    tR += `> ${definition}`;
  } else {
    tR += "> 待补充";
  }
%>

## 相关论文

![[概念检索论文.base]]

## 相关想法

```dataviewjs

let folderChoicePaths = ["00 - 每日日记/DailyNote", "Inputs", "Outputs", "Projects"];
const specificTag = "#想法"; // 指定要检查的标签，可以更改

const files = app.vault.getMarkdownFiles().filter(file => folderChoicePaths.some(path => file.path.includes(path)) );

let names = dv.current().aliases ? dv.current().aliases : [];
names.push(dv.current().name);
names.push(dv.current().ch);

let arr = files.map(async(file) => {
    const content = await app.vault.cachedRead(file);
    // 确保文件内容包含特定标签
    if (content.includes(specificTag)) {
        let lines = content.split("\n").filter(line => names.some(name => line.includes(name)));
        return ["[[" + file.name.split(".")[0] + "]]", lines];
    }
    return null; // 如果不包含标签，返回 null
});

Promise.all(arr).then(values => {
    // 过滤掉 null 结果
    const filteredValues = values.filter(value => value != null);

    const beautify = filteredValues.map(value => {
        const temp = value[1].map(line => line); // 美化处理
        return [value[0], temp];
    });

    const exists = beautify.filter(value => value[1][0] && value[0] != "[[未命名 10]]")
        .sort((a, b) => a[0].localeCompare(b[0])); // 排序处理修正为 localeCompare

    dv.table(["日期", "动态"], exists);
});

```
